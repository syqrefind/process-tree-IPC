...
int main(int argc, const char * argv[]) {
    ...
    act2.sa_handler=&handler_sigstop;
    ...
    if (pid==0){//sum
      ...
      if(pid1==0){//sum 1
        ...
        time_t wait_time = time(NULL) + 3;
        while(time(NULL) <= wait_time){
            if(sigstop_count >= 1){
                long long_pid;
                FILE *output = fopen("kill.txt", "r");
                fscanf (output, "%ld", &long_pid);
                fclose(output);
                kill((pid_t)long_pid,SIGKILL);
            }
        }
      }
      else if (pid1>0){//sum
          ...
          while(time(NULL) <= wait_time){
            ...
            while(1){
                ...
                if (time(NULL)==endwait){
                    // The parent has waited for 3 seconds!!!
                    // Get unfinished children's pids
                    // pid1 = sum1; pid5 = sum2
                    if (sigusr1_count!=1){
                        // sum 1 not finished
                        // ask sum 2 to send kill to sum 1
                        // write to file the pid needed to be killed
                        FILE *output= fopen("kill.txt", "w+");
                        fprintf(output, "%ld\n", (long)pid1);
                        fclose(output);
                        // signals other children to kill
                        if(kill(getppid(), SIGSTOP) == 0) {
                        ...
                        // and send kill by itself
                        kill(pid1, SIGKILL);
                    }
                    if (sigusr2_count!=1){
                        // sum 2 not finished
                        // ask sum 1 to send kill to sum2
                        // write to file the pid needed to be killed
                        FILE *output= fopen("kill.txt", "w+");
                        fprintf(output, "%ld\n", (long)pid5);
                        fclose(output);
